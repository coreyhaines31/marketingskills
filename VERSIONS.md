...
- Added `marketing-loops` skill — a library of ~20 repeatable marketing loops (recurring agent workflows an agent runs on a cadence, each with a trigger, ordered loop body, self-check, and explicit stop/bail-out condition). Inspired by ForwardFuture's Loop Library pattern but scoped to marketing operations, and positioned as the operational cousin of `marketing-ideas` (ideas = what to try once; loops = what to keep doing on a schedule). SKILL.md covers the eight-part loop anatomy, a cadence rule (match frequency to how fast the signal actually changes), when NOT to loop (strategy/creative work, unreviewed spend/publish, sparse signals, vanity loops), and scheduling guidance that defers the mechanics to `loopify` (Claude Code `/loop` / `ScheduleWakeup` / `CronCreate`) or plain cron. `references/loop-catalog.md` holds the full library organized by AARRR + ongoing ops — Acquisition (keyword-gap, ad-fatigue, content-repurposing, competitor-watch, newsjacking), Activation (onboarding drop-off, signup-funnel-leak), Retention (churn-signal, lifecycle-email-refresh, re-engagement), Revenue (pricing-page-experiment, paywall-optimization), Referral (referral-nudge, review-harvest), and Ongoing Ops (weekly marketing review, ranking-drop watch, experiment-backlog, social-listening, content-decay). Each entry orchestrates existing skills (`seo-audit`, `ads`, `emails`, `churn-prevention`, `pricing`, `referrals`, `ab-testing`, `analytics`, etc.). Includes a banned-vocabulary list (no "set it and forget it" / "fully autonomous marketing" / "10x on autopilot"). Total skills: 46.
