...
- **repo**: added OpenAI Codex plugin support (#295). The repo is now installable through Codex's plugin system: new `.codex-plugin/plugin.json` manifest (points at the existing `skills/` directory — no skill files changed) and new `.agents/plugins/marketplace.json` marketplace file so `codex plugin marketplace add coreyhaines31/marketingskills` works. `.gitignore` narrowed from `.agents/` to `.agents/*` with a `!.agents/plugins/` exception so the marketplace file is tracked while installed skills stay ignored. README Installation gains an OpenAI Codex Plugin option; later options renumbered. No skill versions changed; Claude Code / skillkit install paths untouched.
